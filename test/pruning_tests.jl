...
@testset "Pruning" begin
    ...
    @testset "delete_entries" begin
        ...
        new_ba = WaspNet.delete_entries(ba, [3], axis=1)
        @test begin
            all(size(new_ba) .== (4,5))
        end
        @test begin
        ...
        new_ba = WaspNet.delete_entries(new_ba, [3], axis=2)
        @test begin
        ...
        new_W = WaspNet.delete_entries(W, [3], axis=1)
        @test begin
            all(size(new_W) .== (4,5))
        end
        @test begin
        ...
        new_W = WaspNet.delete_entries(new_W, [3], axis=2)
        @test begin
    ...
    @testset "Layer Pruning" begin
        # Matrix W
        neurons = [WaspNet.Izh() for _ in 1:5]
